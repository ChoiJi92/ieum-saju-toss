...
import {
  prepareSpiritCard,
  shareSpiritCard,
  type PreparedSpiritCard,
} from "../lib/spirit-card";
import {
  todaySpirit,
  gunghapScore,
  catchChance,
...
const saveDexClaimed = (nums: number[]) => {
  try {
    localStorage.setItem(DEX_MILESTONE_STORE_KEY, JSON.stringify(nums));
  } catch {
    /* ignore */
  }
};
...
const saveMissionClaimedToday = () => {
  try {
    localStorage.setItem(
      MISSION_STORE_KEY,
      JSON.stringify({ date: todayYmd(), claimed: true }),
    );
  } catch {
    /* ignore */
  }
...
export default function AppShell() {
  ...
  const [flow, setFlow] = useState<FlowScreen[] | null>(() => {
    ...
    try {
      if (!localStorage.getItem("ieum-saju.v2-welcome.v1")) return ["reveal"];
    } catch {
      /* ignore */
    }
    return null;
  ...
  const unlock = (r: Route) => {
    setAdUnlocked((s) => {
      ...
      try {
        localStorage.setItem(
          REWARDED_STORE_KEY,
          JSON.stringify({ date: todayYmd(), routes: [...next] }),
        );
      } catch {
        /* ignore */
      }
  ...
  useEffect(() => {
    ...
    void detectEntryShare().then((share) => {
      ...
      if (share && myeongsik && !flow) {
        setStack((s) =>
          s[s.length - 1] === "gunghap" ? s : [...s, "gunghap"],
        );
  ...
  useEffect(() => {
    if (myeongsik) {
      const t = window.setTimeout(() => {
        void preloadRewardedAdForResult();
      }, 2000);
  ...
  const resetApp = () => {
    ...
    reset();
    try {
      localStorage.removeItem("ieum-saju.spirit.v2");
      localStorage.removeItem("ieum-saju.streak.v1");
      localStorage.removeItem("ieum-saju.v2-welcome.v1");
      localStorage.removeItem(REWARDED_STORE_KEY);
      localStorage.removeItem(DEX_MILESTONE_STORE_KEY);
  ...
  const back = () => {
    if (flow) {
      setFlow((f) => (f && f.length > 1 ? f.slice(0, -1) : f));
      return;
    }
        ...
        >
          <div
            style={{
              ...
              height: 44,
              borderRadius: "50%",
              border: "2.5px solid rgba(183,156,255,.25)",
              borderTopColor: "var(--v2-lavender)",
              animation: "v2-spin-slow 0.8s linear infinite",
            }}
          />
        </div>
      </V2Screen>
  ...
  if (guestShareView && !myeongsik) {
    ...
    return (
      <ScreenGunghap
        go={startOnboard}
        back={startOnboard}
        switchTab={startOnboard}
        spirit={spirit}
        tab={"home" as Tab}
        guestOnboard={startOnboard}
      />
    );
  }
  ...
  switch (route) {
    case "today":
      screenEl = <ScreenToday {...sp} />;
      break;
    case "month":
      screenEl = <ScreenMonth {...sp} />;
      break;
    case "year":
      screenEl = <ScreenYear {...sp} />;
      break;
    case "love":
      screenEl = <ScreenLove {...sp} />;
      break;
    case "money":
      screenEl = <ScreenMoney {...sp} />;
      break;
    case "career":
      screenEl = <ScreenCareer {...sp} />;
      break;
    case "health":
      screenEl = <ScreenHealth {...sp} />;
      break;
    case "gunghap":
      screenEl = <ScreenGunghap {...sp} />;
      break;
    case "jamidusu":
      screenEl = <ScreenJamidusu {...sp} />;
      break;
    case "sinsal":
    ...
    case "fortunes":
      ...
      break;
    case "calendar":
      screenEl = <ScreenCalendar {...sp} />;
      break;
    case "collection":
      screenEl = <ScreenCollection {...sp} />;
      break;
    case "profile":
      screenEl = <ScreenProfile {...sp} />;
      break;
    case "profiles":
      screenEl = <ScreenProfiles {...sp} />;
      break;
    case "addProfile":
      screenEl = <ScreenAddProfile {...sp} />;
      break;
    case "terms":
      screenEl = <ScreenLegal kind="terms" {...sp} />;
      break;
    case "privacy":
      screenEl = <ScreenLegal kind="privacy" {...sp} />;
      break;
    default:
      screenEl = <HomeOrEgg key={spirit.key} {...sp} />;
  }
  if (REWARDED_ROUTES.includes(route)) {
    screenEl = (
      <RewardedGate
        ...
        back={back}
        spirit={spirit}
        unlocked={adUnlocked.has(route)}
        onUnlock={() => unlock(route)}
      >
        {screenEl}
      </RewardedGate>
    );
  ...
  return (
    ...
    >
      <div key={route} style={{ position: "absolute", inset: 0 }}>
        {screenEl}
      </div>
      {adToast && (
        <div
          style={{
            ...
            border: "1px solid var(--v2-butter)",
            color: "var(--v2-butter)",
            fontSize: 12.5,
            fontWeight: 800,
            padding: "8px 16px",
            borderRadius: 999,
            animation: "v2-rise-soft .4s ease",
            pointerEvents: "none",
            whiteSpace: "nowrap",
          }}
        >
    ...
    <V2Screen seed={3}>
      <div style={{ minHeight: 732, display: "flex", flexDirection: "column" }}>
        ...
        <Rise delay={80}>
          <div
            className="v2-cap"
            style={{ textAlign: "center", color: "var(--v2-lavender)" }}
          >
            이음사주 · SPIRIT
          </div>
        </Rise>
          ...
          <div style={{ position: "relative", width: 206, height: 206 }}>
            <div
              style={{
                ...
                borderRadius: "50%",
                background:
                  "radial-gradient(circle at 50% 50%, rgba(214,198,255,.92), rgba(183,156,255,.4) 46%, transparent 72%)",
                animation: "v2-breathe 5s ease-in-out infinite",
                filter: "blur(2px)",
              }}
            />
            <div
              style={{
                ...
                inset: "18%",
                borderRadius: "50%",
                border: "1.5px dashed rgba(183,156,255,.5)",
                animation: "v2-spin-slow 30s linear infinite",
              }}
            />
            <div
              style={{
                ...
                inset: "30%",
                borderRadius: "50%",
                border: "1px dashed rgba(255,158,130,.45)",
                animation: "v2-spin-slow 22s linear infinite reverse",
              }}
            />
            <Sparkles col="#FFD27A" />
            <div
              style={{
                ...
                fontSize: 46,
                color: "#fff",
                textShadow: "0 0 24px rgba(183,156,255,.9)",
              }}
            >
              命
            </div>
        ...
        <Rise delay={220}>
          ...
          >
            당신의 사주엔
            <br />
            <span style={{ color: "var(--v2-lavender)" }}>정령</span>이 깃들어
            있어요
          </h1>
          <p
            className="v2-body"
          ...
          >
            태어난 순간의 오행과 12지를 풀어 오직 당신만의 정령을 깨워드려요.
            매일 함께 운을 살피며 키워보세요.
          </p>
        </Rise>
        <Rise delay={340}>
          ...
          <button
            ...
            style={{
              ...
              alignItems: "center",
              justifyContent: "center",
              boxShadow: "0 8px 24px rgba(183,156,255,.3)",
            }}
          >
            {busy ? "연결 중…" : "토스로 로그인하기"}
          </button>
          ...
          <button
            ...
            style={{
              ...
              border: "1.5px solid var(--v2-glass-line2)",
              cursor: "pointer",
              fontFamily: "var(--v2-font)",
              fontSize: 14.5,
              fontWeight: 700,
...
const SIJIN_HOUR_V2: Record<string, number> = {
  子: 0,
  丑: 2,
  寅: 4,
  卯: 6,
    ...
    <>
      ...
      >
        <span>
          {unknownTime
            ? "태어난 시간 — 모름"
            : SIJIN_LIST_V2.find(([k]) => k === sijin)?.[1]}
        </span>
      ...
      {open && (
        <BottomSheet onClose={() => setOpen(false)} maxHeight="76dvh">
          {[
            ["__unknown", "모름 (시간을 몰라요)"] as [string, string],
            ...SIJIN_LIST_V2,
          ].map(([k, lbl]) => {
            const sel =
              (k === "__unknown" && unknownTime) ||
              (k !== "__unknown" && !unknownTime && sijin === k);
            return (
              <div
                key={k}
                onClick={() => {
                  onPick(
                    k === "__unknown"
                      ? { sijin, unknownTime: true }
                      : { sijin: k, unknownTime: false },
                  );
                  setOpen(false);
                }}
                style={{
                  ...
                  gap: 10,
                  alignItems: "center",
                  background: sel ? "rgba(183,156,255,.12)" : "transparent",
                  color: sel ? "var(--v2-ink)" : "var(--v2-ink-mid)",
                  fontSize: 15,
                  fontWeight: 700,
                }}
              >
                <span style={{ width: 16, color: "var(--v2-lavender)" }}>
                  {sel ? "✓" : ""}
                </span>
                {lbl}
              </div>
...
function ScreenInput({
  ...
  const submit = () => {
    ...
    const input = {
      ...
      day: dNum,
      calendar: cal,
      leapMonth: cal === "lunar" ? leap : false,
      hour: unknownTime ? undefined : SIJIN_HOUR_V2[sijin],
      minute: 0,
      gender,
    };
    ...
    if (cal === "lunar") {
      try {
        computeMyeongsik(input);
      } catch {
        setCalErr(
          "이 음력 날짜는 변환이 어려워요. 양력 날짜로 입력해 주시겠어요?",
        );
        return;
      }
    }
  ...
  return (
    <V2Screen seed={5}>
      ...
      <div style={{ display: "flex", gap: 5, marginTop: 20, marginBottom: 26 }}>
        {[1, 2].map((i) => (
          <div
            key={i}
            style={{
              ...
              borderRadius: 2,
              background:
                i <= 1 ? "var(--v2-lavender)" : "rgba(255,255,255,.12)",
              boxShadow: i <= 1 ? "0 0 10px var(--v2-lavender)" : "none",
            }}
          />
        ))}
      </div>
      <Rise>
        <div className="v2-cap" style={{ color: "var(--v2-peach)" }}>
          STEP 1 / 2 · 사주 정보
        </div>
        <h2 className="v2-hero" style={{ margin: "10px 0 6px" }}>
          언제, 이 세상에
          <br />
          오셨나요?
        </h2>
        <p
          className="v2-body"
          style={{ color: "var(--v2-ink-dim)", margin: "0 0 24px" }}
        >
          태어난 시각이 정확할수록 정령이 또렷해져요 ✦
        </p>
      </Rise>
      <Rise delay={120}>
        <div
              ...
              style={{
                ...
                background:
                  cal === k
                    ? "linear-gradient(120deg,var(--v2-lavender),var(--v2-peach))"
                    : "transparent",
                color: cal === k ? "#1b1230" : "var(--v2-ink-dim)",
              }}
            >
              {t}
            </button>
                ...
                style={{
                  ...
                  background:
                    gender === k
                      ? "linear-gradient(120deg,var(--v2-lavender),var(--v2-peach))"
                      : "var(--v2-glass)",
                  color: gender === k ? "#1b1230" : "var(--v2-ink)",
                }}
              >
                {t}
              </button>
    ...
    return (
      <V2Screen seed={5}>
        ...
        <div style={{ textAlign: "center", marginTop: 80 }}>
          <p
            className="v2-body"
            style={{ color: "var(--v2-ink-dim)", marginBottom: 20 }}
          >
            정보를 불러오지 못했어요.
          </p>
    ...
    <V2Screen seed={5}>
        ...
        {[1, 2].map((i) => (
          <div
            ...
            style={{
              ...
              boxShadow: "0 0 10px var(--v2-lavender)",
            }}
          />
        ))}
      </div>
      <Rise>
        ...
        <p
          ...
          style={{ color: "var(--v2-ink-dim)", margin: "0 0 24px" }}
        >
          시간까지 알면 정령이 더 또렷해져요. 모르면 건너뛰어도 괜찮아요 ✦
        </p>
      </Rise>
      <Rise delay={120}>
        {/* 토스에서 가져온 정보 (읽기 전용) */}
        <V2Glass style={{ marginBottom: 14 }}>
          ...
          >
            ...
            <span style={{ color: "var(--v2-ink-dim)" }}>
              {tossPending.year}.{String(tossPending.month).padStart(2, "0")}.
              {String(tossPending.day).padStart(2, "0")} (양력)
            </span>
      ...
      <div style={{ marginTop: 24 }}>
        <V2Button onClick={submit}>정령 깨우기 ✦</V2Button>
      </div>
    </V2Screen>
  );
}
...
const EGG_BG =
  "radial-gradient(ellipse 72% 82% at 40% 30%, #fffdf8 0%, #f1eaff 58%, #cdbaf2 100%)";
const EGG_RADIUS = "50% 50% 50% 50% / 60% 60% 42% 42%";
// 매끈하게 — 드롭섀도 + 바닥 음영만 (외곽 글로우 제거 → 알 뒤 '원' 헤일로 안 생기게)
const EGG_SHADOW =
  "0 18px 34px rgba(0,0,0,.35), inset -16px -20px 36px rgba(140,108,200,.16)";
// 갈지자 균열선 — 위/아래 껍데기 clip-path (맞물려 알을 이룸)
const SHELL_TOP_CLIP =
  "polygon(0 0,100% 0,100% 44%,85% 50%,70% 44%,55% 51%,45% 44%,30% 51%,15% 45%,0 50%)";
const SHELL_BOT_CLIP =
  "polygon(0 50%,15% 45%,30% 51%,45% 44%,55% 51%,70% 44%,85% 50%,100% 44%,100% 100%,0 100%)";

/** 공용 알 비주얼 — 계열/정체 힌트 없는 중립 코스믹 알 (애니메이션은 호출부가 래퍼로) */
function EggVisual({ size = 150 }: { size?: number }) {
  const w = size,
    h = Math.round(size * 1.25);
  return (
    <div
      style={{
        width: w,
...
function CrackingEgg({ size = 170 }: { size?: number }) {
  ...
  const piece: React.CSSProperties = {
    position: "absolute",
    inset: 0,
    width: w,
    height: h,
    ...
    >
      ...
      <div
        style={{
          ...
          borderRadius: "50%",
          background:
            "radial-gradient(circle, #fff, rgba(255,236,180,.7) 40%, transparent 70%)",
          animation: "v2-hatch-burst 1.9s ease-out forwards",
          filter: "blur(2px)",
          pointerEvents: "none",
        }}
      />
      ...
      <div
        style={{
          ...piece,
          clipPath: SHELL_BOT_CLIP,
          WebkitClipPath: SHELL_BOT_CLIP,
          animation: "v2-shell-bottom 1.9s cubic-bezier(.4,0,.5,1) forwards",
        }}
      />
      {/* 위 껍데기 — 날아오름 */}
      <div
        style={{
          ...piece,
          clipPath: SHELL_TOP_CLIP,
          WebkitClipPath: SHELL_TOP_CLIP,
          animation: "v2-shell-top 1.9s cubic-bezier(.4,0,.5,1) forwards",
        }}
      />
      {/* 파편 */}
      {shards.map((s, i) => (
        <span
          ...
          style={{
            ...
            ["--sy" as string]: s.sy,
            ["--sr" as string]: s.sr,
            animation: "v2-shard-fly 1.9s ease-out forwards",
            pointerEvents: "none",
          }}
        />
      ))}
    ...
    <V2Screen seed={15}>
      ...
      >
        ...
        <Rise style={{ paddingTop: 38 }}>
          <div style={{ textAlign: "center" }}>
            <div className="v2-cap" style={{ color: "var(--v2-lavender)" }}>
              정령 탄생 · {Math.min(done + 1, 3)}/3
            </div>
            <div
              style={{
                marginTop: 7,
        ...
        >
          <div
            ...
            style={{
              animation:
                wiggle > 0
                  ? "v2-egg-wiggle .5s ease"
                  : "v2-egg-float 3s ease-in-out infinite",
            }}
          >
            <EggVisual size={120} />
          </div>
          <div
            className="v2-cap"
            style={{ color: "var(--v2-lavender)", marginTop: 20 }}
          >
            정령의 알 🥚
          </div>
        </Rise>
              ...
              {[0, 1, 2].map((i) => (
                <span
                  ...
                  style={{
                    ...
                    borderRadius: "50%",
                    background:
                      i < done ? "var(--v2-butter)" : "rgba(255,255,255,.15)",
                    boxShadow: i < done ? "0 0 10px var(--v2-butter)" : "none",
                  ...
                  >
                    <span
                      style={{
                        ...
                        justifyContent: "center",
                        fontSize: 21,
                        boxShadow: used ? "none" : `0 0 16px ${a.c}26`,
                      }}
                    >
                      {a.ic}
                    </span>
                    ...
                    <span
                      style={{
                        fontSize: 11.5,
                        fontWeight: 800,
                        color: used ? "var(--v2-mint)" : "var(--v2-ink-dim)",
                      }}
                    >
                      {used ? "완료" : "교감"}
                    </span>
        ...
        >
          <div
            style={{
              ...
              inset: 0,
              background:
                "radial-gradient(circle at 50% 46%, rgba(255,236,180,.5), transparent 62%)",
              animation: "v2-flash 1.6s ease forwards",
              pointerEvents: "none",
            }}
          />
          <div style={{ position: "relative", textAlign: "center" }}>
            ...
            <div
              ...
              style={{
                color: "var(--v2-butter)",
                marginTop: 18,
                animation: "v2-twinkle 0.6s ease-in-out infinite",
              }}
            >
              부화하는 중…
            </div>
      ...
      {phase === "reveal" && (
        ...
        >
          <div
            style={{
              animation: "v2-pop-tf .8s cubic-bezier(.2,1.3,.4,1) both",
            }}
          >
            <SpiritSlot spirit={spirit} size={220} tag={false} stage={1} />
          </div>
          <div
            style={{ animation: "v2-rise-tf .6s ease .25s both", marginTop: 8 }}
          >
            <div style={{ display: "inline-flex", marginBottom: 10 }}>
              <RarityStars rarity={spirit.rarity} />
            </div>
            <h1 className="v2-display" style={{ margin: "0 0 8px" }}>
              {spirit.name}
            </h1>
            <div
              className="v2-cap"
              style={{ color: spirit.elem.raw, marginBottom: 12 }}
            >
              {spirit.formula}
            </div>
            <p
              className="v2-body"
              style={{ color: "var(--v2-ink-dim)", margin: 0, maxWidth: 340 }}
            >
              {spirit.persona}
            </p>
          </div>
          <div
            style={{
              ...
              maxWidth: 340,
              marginTop: 22,
              animation: "v2-rise-tf .6s ease .5s both",
            }}
          >
            <V2Button onClick={onComplete}>
              {spirit.name}에게 첫 운세 듣기 →
            </V2Button>
          </div>
        </div>
      )}
...
function ScreenReveal({
  ...
  useEffect(() => {
    ...
    seq.forEach((ms, i) => {
      acc += ms;
      timersRef.current.push(window.setTimeout(() => setPhase(i + 1), acc));
    });
  ...
  const glyphs = myeongsik
    ...
    : [
        { c: "丙", col: ELEMENTS.fire.raw },
        { c: "子", col: ELEMENTS.water.raw },
        { c: "庚", col: ELEMENTS.metal.raw },
        { c: "寅", col: ELEMENTS.wood.raw },
        { c: "乙", col: ELEMENTS.wood.raw },
        { c: "卯", col: ELEMENTS.wood.raw },
        { c: "戊", col: ELEMENTS.earth.raw },
        { c: "辰", col: ELEMENTS.earth.raw },
      ];
  const core = [
    {
      size: 64,
      bg: "radial-gradient(circle at 40% 36%, #fff, rgba(214,198,255,.5))",
      glow: "rgba(183,156,255,.6)",
    },
    {
      size: 92,
      bg: "radial-gradient(circle at 40% 36%, #fff, var(--v2-lavender))",
      glow: "rgba(183,156,255,.9)",
    },
      ...
      >
        ...
        >
          ...
          >
            <div
              style={{
                ...
                background: core.bg,
                boxShadow:
                  core.glow === "transparent"
                    ? "none"
                    : `0 0 60px ${core.glow}`,
                transition:
                  "width .8s cubic-bezier(.3,.8,.3,1), height .8s cubic-bezier(.3,.8,.3,1), box-shadow .8s ease",
                animation: "v2-breathe 2.6s ease-in-out infinite",
                opacity: phase === 3 ? 0 : 1,
              }}
            />
          </div>
          ...
          <div style={layer(phase === 0)}>
            <div
              style={{
                ...
                marginLeft: -120,
                marginTop: -120,
                animation: "v2-spin-slow 22s linear infinite",
              }}
            >
              {glyphs.map((g, i) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      top: "50%",
                      left: "50%",
                      transform: `translate(-50%,-50%) translate(${Math.cos(a) * 118}px, ${Math.sin(a) * 118}px)`,
                      fontFamily: "var(--v2-serif)",
                      fontSize: 28,
                      fontWeight: 700,
                      color: g.col,
                      textShadow: `0 0 16px ${g.col}`,
                    }}
                  >
                    {g.c}
                  </div>
          ...
          <div style={layer(phase === 1)}>
            <div
              style={{
                ...
                marginLeft: -100,
                marginTop: -100,
                animation: "v2-spin-slow 16s linear infinite reverse",
              }}
            >
              {elems.map((e, i) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      top: "50%",
                      left: "50%",
                      transform: `translate(-50%,-50%) translate(${Math.cos(a) * 82}px, ${Math.sin(a) * 82}px)`,
                      width: 50,
                      height: 50,
                      borderRadius: "50%",
                      background: `${e.raw}33`,
                      border: `2px solid ${e.raw}`,
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      color: e.raw,
          ...
          <div style={layer(phase === 2 || phase === 3)}>
            <div style={{ animation: "v2-egg-float 3s ease-in-out infinite" }}>
              <EggVisual size={128} />
            </div>
          </div>
        </div>
        ...
        <div style={{ minHeight: 128, maxWidth: 340, position: "relative" }}>
          ...
          {phase === 3 && (
            <div style={{ animation: "v2-rise-tf .6s ease .25s both" }}>
              <h1 className="v2-display" style={{ margin: "0 0 10px" }}>
                정령의 알이 맺혔어요
              </h1>
              <p
                className="v2-body"
                style={{ color: "var(--v2-ink-dim)", margin: 0 }}
              >
                세 가지 교감으로 알을 깨우면
                <br />
                정령이 오늘의 운세를 들려줘요 ✦
...
function RevealCaption({
  ...
  return (
    <div key={cap} style={{ animation: "v2-rise-tf .55s ease both" }}>
      <div className="v2-cap" style={{ color }}>
        {cap}
      </div>
      <div
        className="v2-hero"
        style={{ marginTop: 10, color: "var(--v2-ink)" }}
      >
        {title}
      </div>
      <div
        ...
        {[0, 1, 2].map((i) => (
          <span
            ...
            style={{
              ...
              animation: `v2-twinkle 1s ease-in-out ${i * 0.2}s infinite`,
            }}
          />
        ))}
      </div>
...
function ScreenProfile({
  ...
  const onBackup = async () => {
    ...
    if (!linked) {
      ...
      setSyncBusy(false);
      if (r === "linked-reload") {
        setSyncMsg("백업을 불러왔어요 — 잠시 후 새로고침돼요 ✦");
        window.setTimeout(() => window.location.reload(), 900);
      } else if (r === "linked")
        setSyncMsg("백업이 연결됐어요 ✦ 이제 자동으로 동기화돼요");
      else if (r === "no-sync-support")
        setSyncMsg("서버 준비 중이에요 — 곧 열릴 예정!");
  ...
  return (
    <V2Screen seed={9}>
      ...
      <Rise>
        <SelfSpiritSlot spirit={spirit} size={210} />
      </Rise>
      <Rise delay={120}>
        <V2Glass
          glow="0 0 28px rgba(183,156,255,.2)"
          style={{ textAlign: "center" }}
        >
          <div className="v2-cap" style={{ color: spirit.elem.raw }}>
            {spirit.formula}
          </div>
          <h1 className="v2-hero" style={{ margin: "8px 0 4px" }}>
            {spirit.name}
          </h1>
          <RarityStars rarity={spirit.rarity} />
          <p
            className="v2-body"
            style={{ color: "var(--v2-ink-dim)", marginTop: 14 }}
          >
            {spirit.persona}
          </p>
        </V2Glass>
              ...
              >
                <div
                  style={{
                    ...
                    overflow: "hidden",
                    background: reached
                      ? `radial-gradient(circle,${spirit.elem.raw}55,${spirit.rarity.raw}33)`
                      : "var(--v2-glass)",
                    border: `1.5px solid ${st === stage ? spirit.elem.raw : "var(--v2-glass-line2)"}`,
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                  }}
                >
                  {reached ? (
                    url ? (
                      ...
                      <span style={{ fontSize: 24 }}>{spirit.zod.emoji}</span>
                    )
                  ) : (
                    <span style={{ fontSize: 20, color: "var(--v2-ink-mute)" }}>
                      ?
                    </span>
                  )}
                </div>
                <div
                  style={{
                    ...
                    marginTop: 5,
                    color:
                      st === stage ? "var(--v2-ink)" : "var(--v2-ink-mute)",
                  }}
                >
                  {reached ? STAGE_KO[st] : "???"}
                </div>
          ...
          {cols.map((c, i) => (
            <div key={i} style={{ textAlign: "center" }}>
              <div
                style={{
                  fontSize: 10,
                  color: c.isSelf ? "var(--v2-lavender)" : "var(--v2-ink-dim)",
                  fontWeight: 800,
                }}
              >
                {c.l}
        ...
        <>
          ...
          <V2Glass>
                  ...
                  <div key={k} style={{ flex: 1, textAlign: "center" }}>
                    ...
                    >
                      <div
                        style={{
                          width: "64%",
                          height: `${Math.max(8, (n / ohMax) * 100)}%`,
                          background: E.raw,
                          borderRadius: 6,
                          boxShadow: `0 0 10px ${E.raw}55`,
                        }}
                      />
                    </div>
                    <div
                    ...
                    <div style={{ fontSize: 10, color: "var(--v2-ink-dim)" }}>
                      {n}
                    </div>
                  </div>
                );
            ...
            {ohStrong.length >= 2 && (
              ...
              >
                <b style={{ color: ELEMENTS[ohStrong[0]].raw }}>
                  {ELEMENTS[ohStrong[0]].ko}
                </b>{" "}
                기운이 가장 강하고,{" "}
                <b
                  style={{ color: ELEMENTS[ohStrong[ohStrong.length - 1]].raw }}
                >
                  {ELEMENTS[ohStrong[ohStrong.length - 1]].ko}
                </b>{" "}
                기운이 약한 편이에요.
              </div>
            )}
        ...
        <>
          ...
          <V2Glass>
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: 999,
                    background:
                      "linear-gradient(90deg, var(--v2-lavender), var(--v2-peach))",
                  }}
                />
              </div>
            </div>
            ...
            >
              ...
              >
                {shin.yongshin.pulie}({shin.yongshin.kr}) —{" "}
                {shin.yongshinReason}
              </div>
            </div>
          </V2Glass>
      ...
      {card && ilganElem && (
        <>
          ...
          <V2Glass onClick={() => go("personality")}>
            ...
            >
              {myeongsik?.ilgan.c}({ilganKr}) · {ilganElem.cn}
              {ilganElem.ko} 일간
            </div>
            <div
              style={{
            ...
            >
              ✦ 정령은 태어난 해의 ‘띠’가 아니라, ‘나’를 뜻하는{" "}
              <b style={{ color: "var(--v2-lavender)" }}>일주(태어난 날)</b>{" "}
              기준이에요. 계열{" "}
              <b style={{ color: spirit.elem.raw }}>{spirit.line}</b>은(는)
              일간({ilganElem.ko}), 동물은 일지로 정해져 띠와 다를 수 있어요.
            </div>
            <div
              style={{
                fontSize: 11,
      ...
      <ActionRow
        ic="🔮"
        label="다른 사주 보기"
        sub={`현재: ${profile?.name ?? "나"} · 총 ${profiles.length}명`}
        onClick={() => go("profiles")}
      />
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
        <ActionRow
          ic="☁️"
          label={linked ? "데이터 백업 — 연결됨" : "데이터 백업"}
          sub={
            syncBusy
              ? "동기화 중…"
              : (syncMsg ??
        ...
        {!confirmReset ? (
          <button onClick={() => setConfirmReset(true)} style={rowBtn}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11.5, color: "var(--v2-ink-dim)" }}>
                입력한 모든 사주·정령 데이터 삭제
              </div>
            </div>
            <span style={{ color: "var(--v2-ink-dim)" }}>›</span>
          ...
          <V2Glass style={{ border: "1px solid var(--v2-rose)" }}>
            ...
            >
              입력한 사주와 정령·도감 데이터가 모두 삭제되고 처음 화면으로
              돌아가요.
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <div style={{ flex: 1 }}>
                <V2Button kind="glass" onClick={() => setConfirmReset(false)}>
                  취소
                </V2Button>
              </div>
              <div style={{ flex: 1 }}>
                <V2Button
                  onClick={resetApp}
                  style={{ background: "var(--v2-rose)", color: "#1b1230" }}
                >
                  탈퇴하기
                </V2Button>
              </div>
        ...
        >
          {FORTUNE_MENU.map((m) => (
            ...
            >
              ...
              <span style={{ fontSize: 10, color: "var(--v2-ink-dim)" }}>
                {m.sub}
              </span>
            </button>
          ))}
...
function CatchModal({
  ...
  const [phase, setPhase] = useState<"ready" | "throwing" | "caught" | "fled">(
    st0.caught ? "caught" : st0.attempts === 0 ? "ready" : "fled",
  );
  ...
  const anim =
    phase === "throwing"
      ? "v2-catch-wobble 1.25s ease-in-out"
      : phase === "caught"
        ? "v2-catch-caught .8s ease"
        : "none";
  return (
    <div
      onClick={phase === "throwing" ? undefined : onClose}
      style={{
        position: "fixed",
        inset: 0,
        zIndex: 95,
    ...
    >
      ...
      >
        ...
        >
          <div
            key={phase}
            style={{ animation: anim, opacity: phase === "fled" ? 0.35 : 1 }}
          >
            <SpiritSlot
              spirit={todaySp}
              size={200}
        ...
        >
          <span
            style={{ fontSize: 18, fontWeight: 800, color: "var(--v2-ink)" }}
          >
            {todaySp.name}
          </span>
        ...
        <div
          style={{ fontSize: 12.5, color: "var(--v2-ink-dim)", marginTop: 5 }}
        >
          나와 궁합 <b style={{ color: "var(--v2-peach)" }}>{gunghap}%</b> ·
          포획 확률 <b style={{ color: "var(--v2-mint)" }}>{chance}%</b>
        </div>

        <div style={{ marginTop: 20 }}>
          {phase === "ready" && (
            <V2Button onClick={doThrow}>✦ 정령 잡기 (남은 {left}회) ✦</V2Button>
          )}
          {phase === "throwing" && (
            <div
              className="v2-cap"
              style={{
                color: "var(--v2-butter)",
                animation: "v2-twinkle .5s ease-in-out infinite",
              }}
            >
              잡는 중…
            </div>
          ...
          {phase === "caught" && (
            <>
              <div
                className="v2-display"
                style={{ color: "var(--v2-mint)", margin: "0 0 6px" }}
              >
                ✨ 도감에 담았어요!
              </div>
              <p
          ...
          {phase === "fled" && (
            <>
              <div
                className="v2-display"
                style={{ color: "var(--v2-peach)", margin: "0 0 6px" }}
              >
                💨 도망갔어요
              </div>
              {left > 0 ? (
                <>
                  <p
                    className="v2-body"
                    style={{ color: "var(--v2-ink-dim)", margin: "0 0 14px" }}
                  >
                    광고를 보면 한 번 더 도전할 수 있어요
                  </p>
                  <V2Button onClick={retryViaAd}>
                    {adLoading
                      ? "광고 여는 중…"
                      : `🎬 광고 보고 한 번 더 (남은 ${left}회)`}
                  </V2Button>
                  <button
                    onClick={onClose}
                    style={{
...
function ScreenToday({
  ...
  const { claimBonus, progressOf, grantBond } = useSpiritState();
  const stage = progressOf(spirit.key).stage;
  const [bonusMsg, setBonusMsg] = useState<string | null>(null);
  const [shareMsg, setShareMsg] = useState<string | null>(null);
  const [preparedFortuneCard, setPreparedFortuneCard] =
    useState<PreparedSpiritCard | null>(null);
  const [fortuneCardPreparationFailed, setFortuneCardPreparationFailed] =
    useState(false);
  ...
  const bodies = fortune
    ? ([
        ["총운", fortune.sections.overall.body],
        ["애정", fortune.sections.love.body],
        ["금전", fortune.sections.money.body],
        ["일·직업", fortune.sections.work.body],
        ["건강", fortune.sections.health.body],
      ] as const)
    : [];
  const shareFortuneCard = async () => {
    if (!fortune || !preparedFortuneCard) return;
    const result = await shareSpiritCard(
      spirit,
      stage,
      fortune.oneLine,
      "fortune",
    ...
    if (result === "downloaded") setShareMsg("오늘 운세 카드를 저장했어요 🖼️");
      ...
      setShareMsg("카드를 만들지 못했어요 — 다시 시도해주세요");
  ...
  useEffect(() => {
    ...
    if (fortuneOneLine) {
      void prepareSpiritCard(spirit, stage, fortuneOneLine, "fortune").then(
        (card) => {
          if (active) setPreparedFortuneCard(card);
        },
        () => {
  ...
  useEffect(() => {
    ...
    if (r.ok && r.gained > 0) {
      setBonusMsg(`+${r.gained} 교감 ✦`);
      const t = window.setTimeout(() => setBonusMsg(null), 2200);
      return () => window.clearTimeout(t);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [spirit.key]);
  ...
  // dateKey 의존 — 자정 넘겨 화면이 살아있어도 다음 렌더에서 오늘 정령/포획 상태로 갱신 (어제 정령에 오늘 날짜로 포획되는 어긋남 방지)
  const dateKey = todayDateKey();
  // eslint-disable-next-line react-hooks/exhaustive-deps -- dateKey는 날짜 변경 시 재계산 트리거
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- catchTick·dateKey는 localStorage 재읽기 트리거
  const catchSt = useMemo(() => todayCatchState(), [catchTick, dateKey]);
  // 이미 보유한 정령이면 잡을 게 없으므로 '인사하기'로 교감 — 하루 1회, 데일리 미션('잡기 시도')도 이걸로 완료
  const doGreet = () => {
    const r = greetToday();
    if (!r.ok) return;
    grantBond(spirit.key, GREET_GAIN);
    setBonusMsg(`+${GREET_GAIN} 교감 🤍`);
    setCatchTick((t) => t + 1);
    window.setTimeout(() => setBonusMsg(null), 2200);
  };
  ...
  if (!fortune) {
    return (
      <V2Screen seed={13} style={{ paddingBottom: 0 }}>
        ...
        >
          사주를 입력하면 오늘의 운세를 풀어드려요
        </div>
        <div style={{ marginTop: 20 }}>
          <V2Button onClick={() => switchTab("home")}>사주 입력하기</V2Button>
        </div>
        <div style={{ height: 96 }} />
      </V2Screen>
    );
  ...
  return (
    <V2Screen seed={13} style={{ paddingBottom: 0 }}>
      ...
      {shareMsg && (
        <div
          ...
          style={{
            ...
            border: "1px solid var(--v2-lavender)",
            color: "var(--v2-ink)",
            fontSize: 12.5,
            fontWeight: 800,
            padding: "8px 16px",
      ...
      <Rise>
        <div style={{ textAlign: "center" }}>
          <div className="v2-cap" style={{ color: "var(--v2-lavender)" }}>
            {dateLabel} · {myeongsik?.ilgan.c}
            {myeongsik ? `(${TG_KR[myeongsik.ilgan.c]})` : ""}일
          </div>
          <SelfSpiritSlot spirit={spirit} size={172} tag={false} />
          <h1 className="v2-hero" style={{ margin: "2px 0 0" }}>
            {fortune.mood}
          </h1>
        </div>
      </Rise>
        ...
        >
          ...
          >
            {todaySp.imageFor(1) ? (
              ...
              <span style={{ fontSize: 26 }}>{todaySp.zod.emoji}</span>
            )}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div
              style={{ fontSize: 11, color: "var(--v2-ink-dim)", marginTop: 2 }}
            >
              {/* caught를 owned보다 먼저 — 잡는 순간 caughtKeys에 들어가 alreadyOwned=true가 되므로 */}
              {catchSt.caught
                ? "오늘 도감에 담았어요 ✨"
                : alreadyOwned
                  ? catchSt.greeted
                    ? "오늘 인사했어요 ✨"
                    : "이미 친한 정령이에요 · 오늘도 인사해요"
          ...
          {/* 인사하기는 '잡을 게 없는 날' 전용 — 오늘 잡은 경우(caught)는 이미 보상·미션 완료라 제외 */}
          {alreadyOwned && !catchSt.caught && !catchSt.greeted && (
            <button
              onClick={doGreet}
              className="v2-press"
              style={{
      ...
      <Rise delay={200}>
        ...
        >
          <ScoreRing score={ring} color="var(--v2-lavender)" />
          <div
            style={{
              flex: 1,
              display: "grid",
          ...
          >
            {dims.map(([l, v, c, ic]) => (
              ...
              >
                <span
                  style={{ color: c, fontSize: 14, fontWeight: 800, width: 16 }}
                >
                  {ic}
                </span>
                <span
                  style={{ fontSize: 11, color: "var(--v2-ink-dim)", flex: 1 }}
                >
                  {l}
                </span>
      ...
      {todayLuck && (
        <Rise delay={240}>
          ...
          >
            ...
            <div
              style={{
                borderLeft: "1px solid var(--v2-glass-line2)",
                borderRight: "1px solid var(--v2-glass-line2)",
              }}
            >
              <div
                style={{
      ...
      {guide && (
        <Rise delay={320}>
          <V2Label>
            {spirit.name}의 풀이 · {["", "아기", "어린", "성체", "영험"][stage]}{" "}
            정령의 눈
          </V2Label>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {stage >= 2 ? (
              <V2Glass>
                ...
                >
                  <div>
                    ...
                    {guide.doList.slice(0, 2).map((t, i) => (
                      <div
                        key={i}
                        style={{
                          fontSize: 12.5,
                  ...
                  <div>
                    ...
                    {guide.avoidList.slice(0, 2).map((t, i) => (
                      <div
                        key={i}
                        style={{
                          fontSize: 12.5,
              ...
              <V2Glass
                onClick={back}
                style={{
                  border: "1px dashed var(--v2-glass-line2)",
                  textAlign: "center",
                  padding: "15px 16px",
                }}
              >
            ...
            {stage >= 3 ? (
              <V2Glass>
                ...
                {(
                  [
                    ["🌅 아침", guide.missions.morning],
                    ["☀️ 점심", guide.missions.noon],
                    ["🌙 밤", guide.missions.night],
                  ] as const
                ).map(([l, t]) => (
                  <div
                    key={l}
                    style={{ display: "flex", gap: 9, marginBottom: 5 }}
                  >
                    <span
                      style={{
                        fontSize: 11.5,
      ...
      <Rise delay={360}>
        <V2Label>오늘의 럭키</V2Label>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {fortune.hashtags.map((h, i) => (
            <StatPill
              key={i}
              label={`#${i + 1}`}
              value={h.replace(/^#/, "")}
              color={
                ["var(--v2-lavender)", "var(--v2-peach)", "var(--v2-mint)"][i]
              }
            />
          ))}
        </div>
        ...
        >
          <button
            ...
            style={{
              ...
              borderRadius: 999,
              cursor: preparedFortuneCard ? "pointer" : "not-allowed",
              fontFamily: "var(--v2-font)",
              fontSize: 16,
              fontWeight: 800,
              display: "flex",
              ...
              color: preparedFortuneCard
                ? "var(--v2-ink)"
                : "var(--v2-ink-dim)",
              opacity: preparedFortuneCard ? 1 : 0.65,
              backdropFilter: "blur(10px)",
            }}
          >
            {preparedFortuneCard ? "오늘 운세 카드 공유" : "카드 준비 중…"}
          </button>
          {fortuneCardPreparationFailed && (
            <>
              ...
              <V2Button
                onClick={() => setFortuneCardRetry((retry) => retry + 1)}
                kind="ghost"
              >
                카드 다시 준비
              </V2Button>
      ...
      <Rise delay={420}>
        <V2Glass
          ...
          style={{
            ...
            gap: 13,
            background:
              "linear-gradient(120deg, rgba(255,158,130,.10), rgba(183,156,255,.08))",
          }}
          glow="0 0 20px rgba(255,158,130,.12)"
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              좋은 날·주의할 날까지 — 이달의 운세 보기
            </div>
          </div>
          <span
            style={{ color: "var(--v2-peach)", fontSize: 16, fontWeight: 800 }}
          >
            ›
          </span>
        </V2Glass>
...
function ScreenPetHome({
  ...
  const name = profile?.name ?? "나";
  const homeFortune = useMemo(
    () => (myeongsik ? todayFortune(myeongsik) : null),
    [myeongsik],
  );
  ...
  const [careGuide, setCareGuide] = useState(() => {
    try {
      return !localStorage.getItem("ieum-saju.guide.care.v1");
    } catch {
      return false;
    }
  });
  const dismissCareGuide = () => {
    setCareGuide(false);
    try {
  ...
  const [nudge, setNudge] = useState(() => {
    try {
      return !localStorage.getItem("ieum-saju.nudge.backup.v1");
    } catch {
      return false;
    }
  });
  ...
  const dismissNudge = () => {
    setNudge(false);
    try {
      localStorage.setItem("ieum-saju.nudge.backup.v1", "1");
    } catch {
      /* ignore */
    }
  };
  ...
  const [menteeKey, setMenteeKey] = useState<string | null>(() => {
    try {
      return localStorage.getItem("ieum-saju.mentor.v1");
    } catch {
      return null;
    }
  });
  const mentee =
    menteeCands.find((c) => c.key === menteeKey) ?? menteeCands[0] ?? null;
  const pickMentee = (k: string) => {
    setMenteeKey(k);
    try {
      localStorage.setItem("ieum-saju.mentor.v1", k);
    } catch {
      /* ignore */
    }
  };
  ...
  const careActions = [
    ...
    },
    {
      ...
      ic: "🌙",
      t: "명상하기",
      amt: ACTION_GAIN.meditate,
      c: "#B79CFF",
    },
  ] as const;
  const nextCare = nextCareAction(tgtProg.actions);
  const showFortuneCareNudge =
    progressOf(spirit.key).bonuses.fortune &&
    nextCare !== null &&
    tgtRem.capLeft > 0 &&
    !(stage >= 4 && !mentee);
  const nextCareLabel = nextCare
    ? careActions.find((action) => action.kind === nextCare)?.t
    : null;
  ...
  const missions = [
    {
      label: "교감 3종 완료",
        ...
        tgtProg.actions.feed && tgtProg.actions.pet && tgtProg.actions.meditate,
      action: scrollToCare,
    },
    {
      label: "오늘의 운세 확인",
      done: progressOf(spirit.key).bonuses.fortune === true,
      action: () => go("today"),
    },
    {
      label: "오늘의 정령 잡기 시도",
      // 이미 보유한 정령인 날은 잡을 수 없으므로 '인사하기'(greeted)로도 완료 인정
      done: catchSt.attempts > 0 || catchSt.caught || catchSt.greeted,
      action: () => go("today"),
    },
  ] as const;
  ...
  const doClaimMission = () => {
    ...
    if (r.ok && r.gained > 0) {
      playFx(r.gained, "🎁");
      showNotice(`🎁 오늘의 미션 완료 — 교감 +${r.gained}`);
    }
  };
  const isLocalhost =
    typeof window !== "undefined" &&
  ...
  const canBypass =
    ...
    isLocalhost;
  const scrollTop = () => {
    (anchorRef.current?.closest(".ie-scroll") as HTMLElement | null)?.scrollTo({
      top: 0,
      behavior: "smooth",
    });
  };
  const playFx = (gained: number, icon: string) => {
    setGain(gained);
    setBurstIcon(icon);
    setPulseKey((k) => k + 1);
    window.setTimeout(() => setGain(null), 1800);
  };
  ...
  const doCare = (kind: "feed" | "pet" | "meditate", icon: string) => {
    const r = careAct(targetKey, kind);
    if (r.ok && r.gained > 0) {
      playFx(r.gained, icon);
      dismissCareGuide();
    }
  };
  ...
  const doEvolve = () => {
    scrollTop();
    setEvolving(true);
    evoTimer.current = window.setTimeout(finishEvolve, 3400);
  };
  // 출석 보너스 — 그날 첫 진입 1회 (멱등). 첫 적립이면 +N 연출.
  useEffect(() => {
    const r = claimBonus(spirit.key, "attend");
    if (r.ok && r.gained > 0) {
      const t = window.setTimeout(() => playFx(r.gained, "🎁"), 500);
      return () => window.clearTimeout(t);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [spirit.key]);
  ...
  useEffect(() => {
    ...
    if (r.milestone) {
      const t = window.setTimeout(() => {
        playFx(r.gained, "🔥");
        showNotice(
          `🔥 ${r.milestone}일 연속 출석! 특별 보상 +${r.gained} 교감`,
        );
      }, 1100);
      return () => window.clearTimeout(t);
    }
    ...
    <V2Screen seed={15} style={{ paddingBottom: 0 }}>
      ...
      >
        ...
        <Rise style={{ paddingTop: 38 }}>
          ...
          >
            <div>
              <div style={{ fontSize: 12, color: "var(--v2-ink-dim)" }}>
                오늘도 함께해요 ✦
              </div>
              <div
                style={{
            ...
            <div style={{ display: "flex", gap: 7, alignItems: "center" }}>
              <div
                style={{
                  padding: "6px 9px",
                  borderRadius: 8,
                  background: "rgba(255,158,130,.14)",
                  border: "1px solid rgba(255,158,130,.3)",
                  color: "var(--v2-peach)",
                  lineHeight: 1.15,
                  whiteSpace: "nowrap",
                  flexShrink: 0,
              ...
              >
                ...
                >
                  {nextMilestone
                    ? `${nextMilestone.daysLeft}일 뒤 +${nextMilestone.reward}`
                    : "최고 기록 진행 중"}
                </div>
              </div>
              ...
              <button
                onClick={async () => {
                  const ol = myeongsik
                    ? todayFortune(myeongsik)?.oneLine
                    : undefined;
                  const r = await shareSpiritCard(spirit, stage, ol);
                  if (r === "downloaded")
                    showNotice("정령 카드를 저장했어요 🖼️");
                  else if (r === "failed")
                    showNotice("카드를 만들지 못했어요 — 다시 시도해주세요");
                }}
            ...
            >
              <V2Glass
                style={{
                  ...
                  gap: 12,
                  background:
                    "linear-gradient(120deg, rgba(183,156,255,.14), rgba(91,217,172,.08))",
                }}
                glow="0 0 20px rgba(183,156,255,.12)"
              >
                <div
                  style={{
                    width: 42,
          ...
          <Rise style={{ marginTop: 12 }}>
            ...
            >
              ...
              <div style={{ flex: 1, lineHeight: 1.4 }}>
                ...
                <div style={{ fontSize: 11, color: "var(--v2-ink-dim)" }}>
                  토스로 연결하면 기기를 바꿔도 그대로예요
                </div>
              </div>
              <button
            ...
            >
              ...
              {pulseKey > 0 && (
                <div
                  ...
                  style={{
                    ...
                    animation:
                      "v2-bond-flash 0.55s cubic-bezier(.2,.8,.3,1) forwards",
                    pointerEvents: "none",
                    zIndex: 1,
                  }}
                />
              ...
              {pulseKey > 0 && (
                <div
                  ...
                  style={{
                    ...
                    animation:
                      "v2-tap-ring 0.9s cubic-bezier(.2,.8,.4,1) forwards",
                    pointerEvents: "none",
                    zIndex: 2,
                  }}
                />
              ...
              <div
                ...
                style={{
                  ...
                  animation: pulseKey
                    ? "v2-bond-squash 0.85s cubic-bezier(.34,1.56,.64,1)"
                    : "none",
                  position: "relative",
                  zIndex: 3,
                }}
              >
                ...
                <div
                  ...
                  style={{
                    ...
                    transformOrigin: "50% 85%",
                    animation: tapKey
                      ? "v2-tap-bounce 0.42s cubic-bezier(.34,1.56,.64,1)"
                      : "none",
              ...
              {tapReact && (
                <div
                  ...
                  style={{
                    ...
                    left: "50%",
                    fontSize: 26,
                    animation: "v2-tap-react 0.65s ease-out forwards",
                    pointerEvents: "none",
                    zIndex: 6,
                  }}
                >
              ...
              {gain !== null && (
                <div
                  ...
                  style={{
                    ...
                    letterSpacing: "-0.5px",
                    animation:
                      "v2-bond-gain 1.75s cubic-bezier(.2,.8,.4,1) forwards",
                    pointerEvents: "none",
                    zIndex: 6,
                    whiteSpace: "nowrap",
                  }}
                        ...
                        {isEmoji ? (
                          ...
                          <div
                            style={{
                              ...
                              background: "#FFFFFF",
                              boxShadow:
                                "0 0 6px #FFFFFF, 0 0 12px rgba(255,255,255,.55)",
                              transform: "translateX(-50%)",
                              animation: `v2-particle-fly ${dur} ${easing} ${delayMs}ms forwards`,
                            }}
                          />
        ...
        {careGuide &&
          stage < 4 &&
          !rem.actions.feed &&
          !showFortuneCareNudge && (
            <div
              style={{
                textAlign: "center",
                marginTop: 8,
                animation: "v2-float 2.2s ease-in-out infinite",
              }}
            >
              <span
                style={{
              ...
              >
                👇 아래 버튼으로 첫 교감을 해보세요 — 매일 교감하면 자라요
              </span>
            </div>
          )}
          ...
          {canEvolve ? (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                fontFamily: "inherit",
                background: "linear-gradient(100deg, #FFD27A, #5BD9AC)",
                color: "#1b1230",
                fontSize: 16,
                fontWeight: 900,
                boxShadow: "0 8px 28px #FFD27A66",
              }}
            >
              ✦ {nextKo}(으)로 진화하기 ✦
            </button>
            ...
            <V2Glass style={{ padding: "12px 16px" }}>
              <BondMeter
                ...
                sub={
                  stage >= 4
                    ? mentee
                      ? `제자 정령에게 기운을 나눠요 · 오늘 +${tgtProg.gainedToday + tgtProg.adsToday * AD_GAIN} 교감했어요`
                      : "최종 진화 완료 — 새 정령을 만나면 기운을 나눠줄 수 있어요 ✦"
              ...
              {stage < 4 && (
                ...
                >
                  다음 진화까지 기운 {remainBond} · 약{" "}
                  {etaDays <= 1 ? "D-1" : `D-${etaDays}`}
                </div>
              )}
              {/* 멘티 선택 (영험 + 후보 2명 이상) */}
              ...
              {stage >= 4 &&
                mentee &&
                tgtProg.bond >= thresholdOf(tgtProg.stage) &&
                tgtProg.stage < 4 && (
                  <button
                    onClick={() => go("profiles")}
                    className="v2-press"
                    style={{
                      ...
                      fontFamily: "var(--v2-font)",
                      background:
                        "linear-gradient(100deg, rgba(255,210,122,.2), rgba(91,217,172,.2))",
                      color: "var(--v2-butter)",
                      fontSize: 12,
                      fontWeight: 800,
                    }}
                  >
                    ✨ {mentee.sp.name} 진화 준비 완료 — 사주 전환해서
                    진화시키기 ›
                  </button>
                )}
              <div
              ...
              {showFortuneCareNudge && nextCareLabel && (
                ...
                >
                  운세를 들었으니 {mentee?.sp.name ?? spirit.name}에게{" "}
                  {nextCareLabel} 해주세요
                </div>
              )}
              <div style={{ display: "flex", gap: 6 }}>
                {careActions.map((a) => {
                  ...
                  const isHandoffAction =
                    showFortuneCareNudge && a.kind === nextCare;
                  const isEmphasized =
                    isHandoffAction || (!showFortuneCareNudge && inWin);
                  const showTimeBonus = inWin;
                  // disabled 대신 탭 피드백 — 무반응(버그 체감) 제거
                  const onTap = () => {
                    if (noTarget) {
                      showNotice(
                        "가장 영험한 모습이에요 — 새 정령을 만나 기운을 나눠보세요 ✦",
                      );
                      return;
                    }
                    if (used) {
                      showNotice(
                        mentee
                          ? `${mentee.sp.name}는 오늘 이 교감을 받았어요 🌙`
                          : "오늘은 이미 교감했어요 · 내일 또 만나요 🌙",
                      );
                      return;
                    }
                    if (tgtRem.capLeft === 0) {
                    ...
                    >
                      {isEmphasized && !off && (
                        <span
                          style={{
                            ...
                            fontWeight: 900,
                            color: "#1b1230",
                            background: isHandoffAction
                              ? "var(--v2-mint)"
                              : "var(--v2-butter)",
                            padding: "2px 7px",
                            borderRadius: 8,
                            whiteSpace: "nowrap",
                            boxShadow: isHandoffAction
                              ? "0 0 10px rgba(91,217,172,.6)"
                              : "0 0 10px rgba(255,210,122,.6)",
                          }}
                        >
                          {isHandoffAction
                            ? "지금 교감"
                      ...
                      <span
                        style={{
                          ...
                          height: 44,
                          borderRadius: 15,
                          background: isHandoffAction ? `${a.c}35` : `${a.c}1f`,
                          color: a.c,
                          display: "flex",
                          alignItems: "center",
                          justifyContent: "center",
                          ...
                          boxShadow: off
                            ? "none"
                            : isHandoffAction
                              ? `0 0 22px ${a.c}88, 0 0 10px rgba(91,217,172,.5)`
                              : isEmphasized
                                ? `0 0 18px ${a.c}55, 0 0 8px rgba(255,210,122,.35)`
                                : `0 0 16px ${a.c}26`,
                          border:
                            isEmphasized && !off
                              ? `1.5px solid ${isHandoffAction ? "rgba(91,217,172,.8)" : "rgba(255,210,122,.55)"}`
                      ...
                      >
                        {used
                          ? "완료"
                          : `+${a.amt + (showTimeBonus ? TIME_BONUS : 0)}${showTimeBonus ? " ✨" : ""}`}
                      </span>
                    </button>
                  );
                })}
              ...
              {!showFortuneCareNudge &&
                (stage < 4 || mentee) &&
                (() => {
                  const nowKind = (["feed", "pet", "meditate"] as const).find(
                    (k) => inActionWindow(k),
                  );
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {w.emoji} 지금은 {w.label}!{" "}
                        <b style={{ fontWeight: 900 }}>{nm}</b> +{TIME_BONUS}{" "}
                        보너스
                      </span>
                    </div>
                  );
              ...
              {notice && (
                <div
                  style={{
                    ...
                    fontWeight: 700,
                    color: "var(--v2-lavender)",
                    animation: "v2-rise-soft .3s ease",
                  }}
                >
                  {notice}
                </div>
              ...
              {(stage < 4 || mentee) && tgtRem.adsLeft > 0 && (
                ...
                >
                  {adLoading
                    ? "광고 여는 중…"
                    : `🎬 광고 보고 +${AD_GAIN} 교감 (오늘 ${tgtRem.adsLeft}회 가능)`}
                </button>
              )}
              {(stage < 4 || mentee) &&
                tgtRem.capLeft === 0 &&
          ...
          <Rise delay={220} style={{ marginTop: 10 }}>
            <V2Glass style={{ padding: "13px 16px" }}>
                ...
                {missions.map((m) => (
                  <div
                    ...
                    style={{
                      ...
                      fontSize: 12.5,
                      fontWeight: 700,
                      color: m.done ? "var(--v2-mint)" : "var(--v2-ink-mute)",
                    }}
                  >
                    <span
                      style={{ fontSize: 13, width: 15, textAlign: "center" }}
                    >
                      {m.done ? "✓" : "○"}
                    </span>
              ...
              {missionClaimed ? (
                ...
                >
                  3개 다 완료하면 교감 +{MISSION_REWARD} 보상을 받아요
                </div>
              )}
            </V2Glass>
          </Rise>
        ...
        >
          <div
            style={{
              position: "absolute",
              inset: 0,
              background: `radial-gradient(circle at 50% 44%, ${spirit.elem.raw}55, transparent 60%)`,
              animation: "v2-flash 1.8s ease forwards",
              pointerEvents: "none",
            }}
          />
          <div style={{ textAlign: "center", position: "relative" }}>
            <div style={{ position: "relative", display: "inline-block" }}>
              ...
              <div
                style={{
                  ...
                  WebkitMaskImage:
                    "radial-gradient(circle, transparent 30%, #000 55%, transparent 78%)",
                  maskImage:
                    "radial-gradient(circle, transparent 30%, #000 55%, transparent 78%)",
              ...
              <div
                style={{
                  ...
                  zIndex: 0,
                  background:
                    "radial-gradient(circle, #fff 0%, #FFE9A8 35%, transparent 70%)",
              ...
              {[0, 320, 640].map((d, i) => (
                <div
                  ...
                  style={{
                    ...
                    border: `2px solid ${spirit.elem.raw}`,
                    transform: "translate(-50%,-50%)",
                    animation: `v2-bond-ring 1.1s cubic-bezier(.2,.8,.4,1) ${d}ms forwards`,
                    pointerEvents: "none",
                    zIndex: 0,
                  }}
                />
              ...
              <div
                style={{
                  position: "relative",
                  zIndex: 1,
                  animation: "v2-evolve-in 1.2s cubic-bezier(.2,.9,.3,1)",
                }}
              >
                <SpiritSlot
                  spirit={spirit}
            ...
            <div
              ...
              style={{
                marginTop: 10,
                color: "var(--v2-ink)",
                textShadow: "0 0 18px rgba(255,227,160,.85)",
                position: "relative",
                zIndex: 2,
                animation: "v2-rise-tf .5s ease .9s both",
              }}
            >
              ✦ 진화! ✦
            </div>
            <div
              ...
              style={{
                ...
                position: "relative",
                zIndex: 2,
                animation: "v2-rise-tf .5s ease 1.1s both",
              }}
            >
              {STAGE_KO[nextStage]}
            </div>
            <div
              style={{
                ...
                position: "relative",
                zIndex: 2,
                animation: "v2-rise-tf .6s ease 2.2s both",
              }}
            >
              화면을 탭하면 계속돼요
            </div>
...
function ScreenCollection({
  ...
  const [dexClaimed, setDexClaimed] = useState<number[]>(() =>
    loadDexClaimed(),
  );
  ...
  const owners = useMemo(() => {
    const m = new Map<
      string,
      { sp: Spirit; ownerId: string; ownerName: string }
    >();
    for (const p of profiles) {
      try {
        const sp = spiritFromMyeongsik(computeMyeongsik(p)); // 일주 기반 진짜 등급 포함
        if (!m.has(sp.key))
          m.set(sp.key, { sp, ownerId: p.id, ownerName: p.name });
      } catch {
        /* skip */
      }
    }
  ...
  const unlocked = useMemo(
    () => new Set([...owners.keys(), ...caught]),
    [owners, caught],
  );
  const cells = ELEM_ORDER.flatMap((ek) =>
    ZOD_ORDER.map((zk) => {
  ...
  const pendingMilestones = DEX_MILESTONES.filter(
    (n) => ownedCount >= n && !dexClaimed.includes(n),
  );
  ...
  const claimDexMilestones = () => {
    ...
    const pending = DEX_MILESTONES.filter(
      (n) => ownedCount >= n && !fresh.includes(n),
    );
    ...
    <V2Screen seed={17} style={{ paddingBottom: 0 }}>
      ...
      <Rise>
        <V2Glass
          ...
          glow="0 0 24px rgba(255,210,122,.16)"
        >
          <ScoreRing
            score={Math.round((ownedCount / 60) * 100)}
            color="var(--v2-butter)"
          />
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 800 }}>
              {ownedCount} / 60 정령을 만났어요
            </div>
            <div
              style={{
                height: 5,
            ...
            >
              <div
                style={{
                  width: `${(ownedCount / 60) * 100}%`,
                  height: "100%",
                  borderRadius: 999,
                  background:
                    "linear-gradient(90deg, var(--v2-butter), var(--v2-peach))",
            ...
            {pendingMilestones.length > 0 ? (
              ...
              >
                🎁 {pendingMilestones[pendingMilestones.length - 1]}마리 달성
                보상 받기 +{pendingReward}
              </button>
            ) : (
              <div
              ...
              >
                {ownedCount >= 60
                  ? "도감 완성! 모든 정령을 만났어요 🎉"
                  : `다음 목표 ${nextGoal}마리 — ${nextGoal - ownedCount}마리 남았어요. 못 만난 정령을 눌러보세요 ✦`}
              </div>
            )}
          </div>
        </V2Glass>
        ...
        >
          ...
          {ELEM_ORDER.map((ek) => (
            <FilterChip
              key={ek}
              active={filter === ek}
              onClick={() => setFilter(ek)}
              label={`${ELEMENTS[ek].cn} ${ELEMENTS[ek].ko}`}
              color={ELEMENTS[ek].raw}
            />
          ))}
        </div>
      </Rise>
      ...
      >
        {cells.map((c, i) => (
          <Rise key={c.key} delay={Math.min(i * 12, 300)}>
            <div
              onClick={() =>
                c.got
                  ? setView(
              ...
              style={{
                ...
                cursor: "pointer",
                background: c.got
                  ? `linear-gradient(160deg, ${c.sp.elem.raw}1c, var(--v2-glass))`
                  : "var(--v2-glass)",
                border: `1px solid ${c.got ? c.sp.elem.raw + "44" : "var(--v2-glass-line2)"}`,
                opacity: c.got ? 1 : c.ready ? 0.85 : 0.5,
                display: "flex",
                flexDirection: "column",
                alignItems: "center",
            ...
            >
              <div
                style={{
                  ...
                  overflow: "hidden",
                  background: c.got
                    ? `radial-gradient(circle at 38% 34%, #fff8, ${c.sp.elem.raw}, ${c.sp.rarity.raw})`
                    : "rgba(255,255,255,.05)",
                  boxShadow: c.got ? `0 0 16px ${c.sp.elem.raw}88` : "none",
                  border: c.got ? "none" : "1px dashed var(--v2-glass-line2)",
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  fontSize: 22,
              ...
              <div
                style={{
                  fontSize: 11.5,
                  fontWeight: 800,
                  color: c.got ? "var(--v2-ink)" : "var(--v2-ink-mute)",
                }}
              >
                {c.got ? c.sp.name : "???"}
              </div>
              ...
              >
                {c.got ? c.sp.formula : c.ready ? "미발견" : "준비중"}
              </div>
              {c.got && (
                <div style={{ marginTop: 4 }}>
                  <RarityStars rarity={c.sp.rarity} showLabel={false} />
                </div>
              )}
              {c.key === spirit.key && (
                <span
            ...
            <BottomSheet onClose={() => setView(null)}>
              <div style={{ textAlign: "center" }}>
                ...
                {caughtOnly ? (
                  ...
                  >
                    {view.sp.formula} · 오늘의 정령으로 도감에 담은 정령 🎯
                  </div>
                ) : (
                  <>
                    <div
                    ...
                    <div style={{ margin: "12px 2px 0" }}>
                      <BondMeter
                        percent={vp.stage >= 4 ? 100 : vpct}
                        label={vp.stage >= 4 ? "기운" : "다음 진화까지"}
                        sub={vp.stage >= 4 ? "최종 진화 완료 ✦" : undefined}
                      />
                    </div>
                  </>
                )}
              ...
              {caughtOnly ? (
                ...
                >
                  이 사주로 태어난 사람을 만나면 직접 키울 수도 있어요 ✦
                </p>
              ) : (
                <button
                  onClick={() => {
                    if (!isActive) setActive(view.ownerId);
                    setView(null);
                    switchTab("home");
                  }}
                  className="v2-press"
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 13 }}>
            <div
              style={{
                ...
                height: 58,
                borderRadius: "50%",
                background: `radial-gradient(circle, ${wish.elem.raw}33, var(--v2-glass))`,
                border: `1.5px dashed ${wish.elem.raw}66`,
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                fontSize: 26,
            ...
            <div style={{ flex: 1 }}>
              ...
              >
                일간{" "}
                <b style={{ color: wish.elem.raw }}>
                  {wish.elem.cn}
                  {wish.elem.ko}
                </b>{" "}
                · 일지{" "}
                <b style={{ color: "var(--v2-lavender)" }}>
                  {wish.zod.ko}({wish.zod.cn})
                </b>
                인 사주에서 태어나요
              </div>
            </div>
