        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 800, color: items[open].color }}>
              {items[open].name} ({items[open].hanja})
            </div>
          </div>
...
function DaewoonCard({ items }: { items: DaewoonItem[] }) {
  return (
    <IECard style={{ marginTop: 14 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 10 }}>
        <div style={{ fontSize: 13, fontWeight: 800 }}>대운 (大運)</div>
...
function SeunCard({ items }: { items: SeunItem[] }) {
  return (
    <IECard style={{ marginTop: 14 }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 10 }}>
        <div style={{ fontSize: 13, fontWeight: 800 }}>세운 (歲運)</div>
        <div style={{ fontSize: 11, color: 'var(--cp-text-dim)', fontWeight: 600 }}>
          매년 흐름 (날씨)
        </div>
      </div>
      ...
      >
        대운이 큰 계절이라면, 세운은 그 안의 매년 날씨예요. 올해는 노란색.
        세운 천간이 일간과 만나면 한 해 흐름의 핵심 키워드가 결정돼요.
      </p>
    </IECard>
  );
}
...
방향성을 정하는 사람과 함께면 시너지가 폭발해요.`,
    癸: `이슬비·맑은 샘의 기운이에요.
차분하고 꾸준해요. 조용히 스며드는 영향력이 있어요.
드러내지 않아도 곁에 있으면 마음이 차분해지는 타입.
