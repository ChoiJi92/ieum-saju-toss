  ...
  return (
    <V2Screen seed={15} style={{ paddingBottom: 0 }}>
      ...
      <Rise delay={100}>{canEvolve
        ? <button onClick={() => evolve(spirit.key)} style={{ width: '100%', marginTop: 6, padding: 16, borderRadius: 'var(--v2-r-md)', border: 'none', cursor: 'pointer', fontFamily: 'inherit', background: 'linear-gradient(100deg, #FFD27A, #5BD9AC)', color: '#1b1230', fontSize: 16, fontWeight: 900, boxShadow: '0 8px 28px #FFD27A66' }}>✦ {nextKo}(으)로 진화하기 ✦</button>
      ...
      <Rise delay={260}><V2Label>오늘의 교감</V2Label><div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
        <CareAction onClick={() => bondUp(spirit.key, 25)} ic="🍃" title="기운 먹이주기" sub="오행의 기운을 나눠줘요" amt="+25" color="var(--v2-mint)" />
        <CareAction onClick={() => bondUp(spirit.key, 20)} ic="✨" title="쓰다듬기" sub="정령과 눈을 맞춰요" amt="+20" color="var(--v2-peach)" />
        <CareAction onClick={() => bondUp(spirit.key, 30)} ic="☾" title="함께 명상하기" sub="고요히 기운을 모아요" amt="+30" color="var(--v2-lavender)" />
      </div></Rise>
      <div style={{ height: 96 }} />
    </V2Screen>
  );
