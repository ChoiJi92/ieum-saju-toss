...
export type Route =
  ...
  | 'gunghap' | 'jamidusu' | 'sinsal' | 'personality' | 'thai'
  | 'report'
...
export const REWARDED_ROUTES: Route[] = ['month', 'year', 'personality', 'sinsal', 'thai'];
...
export const ROUTE_TITLE: Record<string, string> = {
  ...
  health: '건강운', gunghap: '궁합', jamidusu: '자미두수', sinsal: '신살', personality: '성격 분석', calendar: '일진 달력',
  report: '정밀 리포트',
  thai: '태국 점성술',
};
...
export const FORTUNE_MENU: { route: Route; ic: string; label: string; sub: string; color: string }[] = [
  ...
  // 유일한 유료 항목. 성격 분석 화면 하단에만 두면 아무도 못 찾는 자리라(내정보 탭 깊숙이)
  // 사람들이 실제로 뭘 볼지 고르는 이 목록 상단에 둔다.
  // 부제는 다른 항목과 같은 결로 — 가격·분량이 아니라 무엇을 다루는지.
  // 유료라는 신호는 금색 테두리와 ✦ 로 주고, 실제 가격은 눌러서 들어간 목차 화면에서 밝힌다.
  { route: 'report', ic: '📜', label: '정밀 리포트', sub: '성격과 지금 시기', color: 'var(--v2-butter)' },
  ...
  { route: 'jamidusu', ic: '🔮', label: '자미두수', sub: '내 명궁의 별', color: 'var(--v2-lavender)' },
  { route: 'sinsal', ic: '🪄', label: '신살', sub: '8가지 기운', color: 'var(--v2-butter)' },
  { route: 'thai', ic: '🐘', label: '태국 점성술', sub: '태어난 요일의 운', color: 'var(--v2-peach)' },
];
